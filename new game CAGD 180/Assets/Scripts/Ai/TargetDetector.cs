...
public class TargetDetector : Detector
{
    ...
    public override void Detect(AiData aiData)
    {
        ...
        Collider[] playerCollider = Physics.OverlapSphere(transform.position, targetDetectionRange, playerLayerMask);

        if (playerCollider != null)
        {
            //Check if you see the player
            Vector3 direction = (playerCollider.transform.position - transform.position).normalized;
            RaycastHit hit = Physics.Raycast(transform.position, direction, targetDetectionRange, obstaclesLayerMask);
